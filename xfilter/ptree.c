...
{
  struct patricia_node *n = xzalloc(sizeof(*n));
  n->l = n->r = NULL;
  return n;
}
...
static int *ptree_search_data_r_p (struct patricia_node *node, char *str, size_t slen, int maxbitplace)
{
  if (node->l == NULL && node->r == NULL)
    return &(node->value.data);
  if (testbit_max(str, node->value.thres_bit, maxbitplace) != 0)
...
static int *ptree_search_data_nearest_ptr (char *str, size_t sstr, struct patricia_node *root)
{
...
ptree_make_root_node (char *str, size_t sstr, int data, struct sockaddr_storage *addr, size_t alen)
{
  struct patricia_node *n = new_node();
  n->value.data = data;
  n->key = xmemdup(str, sstr);
  n->klen = sstr;
  if (addr)
    n->addr = xmemdup(addr, alen);
  else
    n->addr = NULL;
  n->alen = alen;
  return n;
                                ...
                                struct patricia_node **parentlink)
{
  ...
  if (node->value.thres_bit > bitloc ||
    ...
    newleaf->key = xmemdup(str, slen);
    newleaf->klen = slen;
    if (addr)
      newleaf->addr = xmemdup(addr, alen);
    else
      newleaf->addr = NULL;
    newleaf->alen = alen;
