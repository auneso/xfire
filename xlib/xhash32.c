/* 
** 2015/01/30 HDS - add
** 2015/04/13 HDS - xh32search
** 2015/11/18 HDS - xh32renew
*/
#include <string.h>
#include "xhash32.h"
#include "hashf.h"
#include "xalloc.h"


/*
 * By Bart Massey, based on Knuth LCG.
 * Do an extra multiply to get the middle 32 bits
 * of the 64 bit product.
 */
static inline unsigned int barthash (unsigned int k)
{
  /* 0x9e375365UL == 40499 * 65543 - both are prime;
     the result is ~0.68*(2^32) --Chuck Lever */

  return (0x9e37 * (k & 0xffff) + 0x5365 * (k >> 16));
}

static void xh32grow (xh32tab  *t)
{
  int     oldsize = t->tsize; 
  int     newsize = prime_next_size(&t->pindex);
  int     i;
  xh32item **oldtab = t->table;
  xh32item **newtab = (xh32item **)xzalloc(newsize * sizeof(xh32item *));

  t->table = newtab;
  t->tsize = newsize;

  /* Walk through old table putting entries in new table */
  for (i = oldsize; i--; ) {
    xh32item *this, *that, **newplace;
    for (this = oldtab[i]; this;) {
      that = this;
      this = this->next;
      newplace = &newtab[(that->hval %  newsize)];
      that->next = *newplace;
      *newplace = that;
    }
  }
  xh32first(t);
  xfree((char *)oldtab);
}

void xh32init (xh32tab *t, int hsize, int dsize)
{
  int tsize;

  tsize = prime_size(hsize, &t->pindex);
  t->table = (xh32item **)xzalloc(sizeof(xh32item *) * (ub32)tsize);
  t->tsize   = tsize;
  t->dsize   = dsize;
  t->count   = 0;
  t->apos    = (ub32)0;
  t->ipos    = (xh32item *)0;
  t->space   = xfmem_create(sizeof(xh32item) + dsize, hsize);
 
#ifdef XHASH32_STATS
  t->add   = t->del = t->collisions = 0;
#endif
} 

xh32tab *xh32create (int hsize, int dsize) 
{
  xh32tab *t = (xh32tab *)xmalloc(sizeof(xh32tab));
  xh32init(t, hsize, dsize);
  return t;
}

void xh32clean (xh32tab  *t)
{
  int new_space = xfmem_max_items(t->space) / 2;
  xfmem_destroy(t->space);

  memset(t->table, 0, sizeof(xh32item *) * (ub32)t->tsize);
  t->space = xfmem_create(sizeof(xh32item) + t->dsize, new_space);
  t->count = 0;
  t->apos    = (ub32)0;
  t->ipos    = (xh32item *)0;

#ifdef XHASH32_STATS
  t->add   = t->del = t->collisions = 0;
#endif
}

void xh32renew (xh32tab *t, int hsize)
{
  xh32free(t); 
  xh32init(t, hsize, t->dsize);
}

void xh32free (xh32tab  *t)
{
  xfmem_destroy(t->space);
  xfree(t->table);
}

void xh32destroy (xh32tab  *t)
{
  xh32free(t);
  xfree(t);
}

int xh32find (xh32tab *t, ub32 key)
{
  xh32item *h;
  ub32  x = barthash(key);
  ub32  y = x % t->tsize;

  for (h = t->table[y]; h; h = h->next) {
    if (key == h->key) {
      t->apos = y;
      t->ipos = h;
      return TRUE;
    }
  }
  return FALSE;
}

void *xh32search (xh32tab *t, ub32 key)
{
  xh32item  *h;
  ub32    x = barthash(key);
  ub32    y = x % t->tsize;

  /* make sure the key is not already there */
  for (h = t->table[y]; h; h = h->next) {
    if (key == h->key) {
      t->apos = y;
      t->ipos = h;
      return h->stuff;
    }
  }
  return NULL;
}

void *xh32get (xh32tab *t, ub32 key, int *new)
{
  xh32item  *h,**hp;
  ub32    x = barthash(key);
  ub32    y = x % t->tsize;

  /* make sure the key is not already there */
  for (h = t->table[y]; h; h = h->next) {
    if (key == h->key) {
      t->apos = y;
      t->ipos = h;
      if (new) *new = 0;
      return h->stuff;
    }
  }
#ifdef XHASH32_STATS
  if (h) t->collisions++;
  t->add++;
#endif
  /* find space for a new item */
  h = (xh32item *)xfmem_malloc(t->space);

  /* make the hash table bigger if it is getting full */
  if (++t->count > t->tsize) {
    xh32grow(t);
    y = (x % t->tsize);
  }

  /* add the new key to the table */
  h->key   = key;
  h->hval  = x;
  hp = &t->table[y];
  h->next = *hp;
  *hp = h;
  t->ipos = h;
  t->apos = y;
  if (new) *new = 1;
  return h->stuff;
}


int xh32del_adjust (xh32tab *t, int adjust)
{
  xh32item  *h;
  xh32item **ip;

  /* check for item not existing */
  if (!(h = t->ipos)) return FALSE;

  /* remove item from its list */
  for (ip = &t->table[t->apos]; *ip != h; ip = &(*ip)->next)
    ;
  *ip = (*ip)->next;
  --(t->count);

  /* adjust position to something that exists */
  if (adjust && !(t->ipos = h->next)) xh32nbucket(t);

  xfmem_free(t->space, h);
#ifdef XHASH32_STATS
  t->del++;
#endif
  return TRUE;
}


int xh32first (xh32tab *t)
{
  t->apos = t->tsize - 1;
  (void)xh32nbucket(t);
  return (t->ipos != (xh32item *)0);
}

int xh32nbucket (xh32tab *t)
{
  ub32  oldapos = t->apos;
  ub32  end = t->tsize;
  ub32  i;

  /* see if the element can be found without wrapping around */
  for (i = oldapos + 1; i < end; ++i) {
    if (t->table[i % t->tsize]) {
      t->apos = i;
      t->ipos = t->table[i];
      return TRUE;
    }
  }

  /* must have to wrap around to find the last element */
  for (i = 0; i <= oldapos; ++i) {
    if (t->table[i]) {
      t->apos = i;
      t->ipos = t->table[i];
      return FALSE;
    }
  }
  return FALSE;
}

void **xh32array (xh32tab *t)
{
  int used = t->count;
  void **table, **tptr;

  if (used == 0) return NULL;
  table = (void **) xmalloc(sizeof(void*)*(used));
  tptr = table;
  if (xh32first(t)) do {
    *tptr++ = xh32stuff(t);
  } while (xh32next(t));

  return table;
}

#ifdef XHASH32_STATS
int xh32ash_stats (xh32tab *t, char* stats, int len) 
{
  float effic =  (float) t->collisions / ((float) t->count + 0.01) * 100.0;

  return snprintf(stats, len, "add:%d,  del:%d, col:%d (%0.1f%%)\n", t->add, t->del, t->collisions, effic);
}
#endif
