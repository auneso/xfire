...
struct json_tokener* json_tokener_new_ex(int depth)
{
  ...
  tok = (struct json_tokener*)xzalloc(sizeof(struct json_tokener));
  tok->stack = (struct json_tokener_srec *)xzalloc(depth * sizeof(struct json_tokener_srec));
  tok->pb = printbuf_new();
  tok->max_depth = depth;
...
static void json_tokener_reset_level(struct json_tokener *tok, int depth)
{
  ...
  xfree(tok->stack[depth].obj_field_name);
...
char* strndup(const char* str, size_t n)
{
  if (str) {
    ...
    char* s = (char*)xmalloc(sizeof(char) * (nn + 1));
