...
usage: send_arp sndr_ip_addr sndr_hw_addr\n\
                ...
                [[targ_interface [src_hw_addr [dest_hw_addr] [request|reply]]\n\n"};
...
int main (int argc,char** argv)
{
  ...
  if (argc >= 8) {
    get_hw_addr((char *)pkt.dest_hw_addr, argv[7]);
  } else {
    get_hw_addr((char *)pkt.dest_hw_addr, argv[4]);
  }

  if (argc >= 7) {
    get_hw_addr((char *)pkt.src_hw_addr, argv[6]);
  } else {
    get_hw_addr((char *)pkt.src_hw_addr, argv[2]);
  }

  get_hw_addr((char *)pkt.targ_hw_addr, argv[4]);
  get_hw_addr((char *)pkt.sndr_hw_addr, argv[2]);
