...
void xfire_fatal (const char *message, ...)
{
  ...
  if ((log_fd = fopen(XFIRE_LOG_FILE, "a+")) == NULL) {
    xfire_warn(XFIRE_LOG_FILE);
    return;
  }  
  time(&now);
...
void
xfire_err(int eval, const char *fmt, ...)
{
  va_list ap;
...
void    
xfire_errx (int eval, const char *fmt, ...)
{       
  va_list ap;
